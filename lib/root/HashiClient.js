...
class HashiClient {
     ...
     */
    #eventManager = new base_2.EventManager(this);
    /**
     * The language manager for accessing strings.
     */
    #languageManager = new base_3.LanguageManager(this);
    /**
     * The database manager for accessing data maps/lakes.
     */
    #databaseManager = new base_4.DatabaseManager(this);
    /**
     * The services manager for accessing different services (automatic roles, etc.).
     */
    #serviceManager = new base_5.ServiceManager(this);
    /**
     * The services manager for accessing different services (automatic roles, etc.).
     */
    #fileManager = new FileManager_1.FileManager(this);
    /**
     * The language manager for accessing strings.
     */
    ...
    constructor(options) {
        ...
        if (options.mongoose.connectionURI)
            this.databaseManager.setConnectionURI(options.mongoose.connectionURI);
    }
    /**
    ...
    async login(token = process.env.TOKEN || process.env.token || process.env.Token) {
        ...
        while (++i < Object.keys(this.databaseManager.dataMaps).length) {
            ...
            if (dataMap.intents.includes(base_6.DATAMAP_INTENTS.CORE))
