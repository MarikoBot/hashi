...
class HashiCommandBase {
    ...
    static async flowControlWall(client, interaction, ctx) {
        ...
        const activeCoolDowns = client.commands.coolDowns.values(interaction.user.id, command.id);
        const activeInterfering = client.commands.interfering.values(interaction.user.id, ...(command.interferingCommands || []));
    ...
    static async flowControlRegister(client, interaction, CommandGroup) {
        client.commands.interfering.registerInterfering(interaction.user.id, CommandGroup.subcommand?.fullName || CommandGroup.command?.fullName, interaction);
        client.commands.coolDowns.registerCoolDown(interaction.user.id, CommandGroup.subcommand?.fullName || CommandGroup.command?.fullName, CommandGroup.subcommand?.coolDown ||
