...
class Command {
    ...
    async callback(client, ctx) {
        new base_1.Logger().log('info', client, ctx);
    ...
    async isAuthorized(interaction, metadata) {
        ...
        const authBooleans = {
            ...
            uniqueUser: !privilegesData.uniqueUsers.length ||
            ...
            uniqueGuild: !privilegesData.uniqueGuilds.length ||
            ...
            uniqueChannel: !privilegesData.uniqueChannels.length ||
                privilegesData.uniqueChannels.includes(interaction.channel.id),
        };
        for (const [authKey, value] of Object.entries(authBooleans)) {
            if (value) {
                privileges =
                    Number(privileges) > Number(_1.bitRecord[`${authKey}s`])
                        ? privileges
                        : _1.bitRecord[`${authKey}s`];
                continue;
            }
            missing.push(`${authKey}s`);
        }
        const highestMissing = missing.sort((a__, b__) => Number(_1.bitRecord[b__]) - Number(_1.bitRecord[a__]))[0];
        const isAuth = missing.length > 0 ? Number(highestMissing) < Number(privileges) : true;
        if (!isAuth) {
            const errorCode = `${missing.length}${missing
                .map((elt) => Number(_1.bitRecord[elt]))
                .reduce((acc, val) => acc + val, 0)}`;
            new base_1.Logger().log('debug', missing);
    ...
    async flowControlWall(client, interaction, ctx, metadata) {
        ...
        if (activeInterfering.length > 0) {
            const interferingList = activeInterfering.map((elt) => `</${elt[0]}:${elt[1].commandId}>`);
...
__decorate([
    decorators_1.StringValidator.ValidNonFormatted,
    __metadata("design:type", String)
], Command.prototype, "description", void 0);
__decorate([
    decorators_1.ArrayValidator.OnlyHashiErrors,
    __metadata("design:type", Array)
], Command.prototype, "errors", void 0);
__decorate([
    decorators_1.ArrayValidator.OnlyObjects,
    __metadata("design:type", Array)
], Command.prototype, "interferingCommands", void 0);
__decorate([
    decorators_1.NumberValidator.Matches,
    __metadata("design:type", Number)
], Command.prototype, "coolDown", void 0);
__decorate([
    decorators_1.ObjectDeepValidator.IsInstanceOf(base_1.Context),
    __metadata("design:type", base_1.Context)
], Command.prototype, "context", void 0);
__decorate([
    decorators_1.ObjectValidator.KeyStringArrayPair,
