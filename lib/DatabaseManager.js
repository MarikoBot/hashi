...
class DatabaseManager {
     ...
     */
    createDataMap(name, technology = DataMap_1.DB_TECHNOLOGY.SQLITE) {
        let dataMap;
        if (technology === DataMap_1.DB_TECHNOLOGY.SQLITE)
            dataMap = new DBSQLite_1.DBSQLite(name);
     ...
     */
    ensure(dataMapName, force = false, technology = DataMap_1.DB_TECHNOLOGY.SQLITE) {
        if (dataMapName in this.dataMaps)
            return this.dataMaps[dataMapName];
        if (force)
            return this.createDataMap(dataMapName, technology);
        return null;
    }
}
