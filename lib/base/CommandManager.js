...
class CommandManager extends _1.BaseClient {
    ...
    authorizationCallback = async (context, errorCode) => {
        await context.reply({
            content: `<:MarikoCross:1191675946353299456> **Error** → missing privileges ` +
    ...
    getCommandFromInteraction(interaction) {
        ...
        const [maybeSub, maybeGroup] = [
            root_1.Client.tryTo(interaction.options.getSubcommand),
            root_1.Client.tryTo(interaction.options.getSubcommandGroup),
        ];
        if (maybeSub) {
            if (maybeGroup)
    ...
    inject(metadata) {
        ...
        return function (target) {
            new _1.Logger().log('info', `Bound command: ${metadata.id}`);
            if (!('src' in metadata))
...
__decorate([
    decorators_1.ObjectDeepValidator.IsInstanceOf(root_1.CoolDownManager),
    __metadata("design:type", root_1.CoolDownManager)
], CommandManager.prototype, "coolDowns", void 0);
__decorate([
    decorators_1.ObjectDeepValidator.IsInstanceOf(root_1.InterferingManager),
    __metadata("design:type", root_1.InterferingManager)
], CommandManager.prototype, "interfering", void 0);
__decorate([
    decorators_1.ObjectDeepValidator.IsInstanceOf(discord_js_1.Collection),
