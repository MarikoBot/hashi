...
class DiscordEventManager extends _1.BaseClient {
    ...
    inject(name, callback) {
        new _1.Logger().log('info', `Bound event: ${name}`);
        const event = new root_1.DiscordEvent(name);
        event.callback = callback;
        this.client.src[name === 'ready' ? 'once' : 'on'](name, (...args) => event.callback(this.client, ...args));
        return event;
    }
}
