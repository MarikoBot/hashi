...
class Context extends _1.BaseClient {
    ...
    async reply(messageData, interaction = this.interaction) {
        ...
        try {
            message =
                (await interaction.reply(messageData).catch(_1.Logger.clean)) ||
                    (await interaction.followUp(messageData).catch(_1.Logger.clean));
            if (!message)
                return null;
    ...
    async editReply(messageData, interaction = this.interaction) {
        ...
        let message;
        try {
            message = await interaction.editReply(messageData).catch(_1.Logger.clean);
            if (!message)
                return null;
            this.replyData = message;
        }
        catch (error) {
            _1.Logger.clean(error);
            return null;
        }
        return message;
    }
...
__decorate([
    decorators_1.Validators.ObjectValidator.Matches,
    __metadata("design:type", discord_js_1.SelectMenuInteraction)
